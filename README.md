...
Note: for macos, you will have to build the binary as a `.dylib` in a `Stronghold_Game.framework` folder. The framework folder should also have a `Resources` folder with a file called `Info.plist`. Without this file, signing will fail.
...
```sh
bin/Stronghold_Game.macos.template_debug.framework/Stronghold_Game.macos.template_release
bin/Stronghold_Game.ios.template_debug.framework/Stronghold_Game.ios.template_release.arm64.dylib
